...
**Demo:** [youtu.be/s3G7wC6YMX4](https://youtu.be/s3G7wC6YMX4)  
...
### 10. MCP Agent — Portfolio MCP Server
An MCP (Model Context Protocol) server that exposes the tools of the Meeting Intelligence Agent to Claude Desktop. Claude can autonomously list meetings, retrieve decisions and action items, search transcripts with semantic similarity, and answer natural language questions via RAG — all directly from a conversation, without touching any UI.

**Tech:** Python · MCP SDK · asyncio · PostgreSQL + pgvector · Claude Haiku  
**[View project →](backend/agents/mcp_agent/)**

---

### 11. Expense Tracker — Personal PWA
Personal expense tracker installable on iPhone as a PWA. Ingests expenses from three unreliable sources — an Apple Shortcuts Wallet automation, bank notification emails, and PayPal emails — parsed and categorized automatically, plus a manual-entry safety net for when the Wallet trigger silently fails (a documented, unresolved Apple bug). Deduplicates the same real purchase when it's reported by both Wallet and the bank email.

**Tech:** Python · FastAPI · PostgreSQL · SQLAlchemy · Claude Haiku · Vanilla JS PWA (manifest + service worker)  
**[View project →](backend/agents/expense_tracker/)**
...
| Tool | Used for |
|---|---|
| Claude Haiku (Anthropic) | Conversational agents, translation, lead capture, analytics, gap reports, meeting extraction, MCP RAG |
...
| MCP SDK | Model Context Protocol server — MCP Agent |
...
| Tool | Used for |
|---|---|
| PostgreSQL + pgvector | Production vector store — Document Intelligence + Meeting Intelligence + MCP Agent |
...
└── agents/
    ...
    ├── mcp_agent/               # MCP server — Claude Desktop integration
    ├── mcp_demo/                # MCP demo server (development)
    └── expense_tracker/         # Personal expense PWA (Docker + PostgreSQL)
...
For the Expense Tracker (requires Docker for PostgreSQL):

```bash
docker-compose up db_expense_tracker --build
# Demo available at http://localhost:8000/expense-tracker/demo
```

For the MCP Agent (Claude Desktop integration):
...
| `MEETING_DATABASE_URL` | Meeting Intelligence Agent + MCP Agent (PostgreSQL port 5433) |
...
| `EXPENSE_TRACKER_DATABASE_URL` | Expense Tracker (PostgreSQL, port 5434) |
| `EXPENSE_TRACKER_POSTGRES_DB` | Expense Tracker (docker-compose db init) |
| `EXPENSE_TRACKER_POSTGRES_USER` | Expense Tracker (docker-compose db init) |
| `EXPENSE_TRACKER_POSTGRES_PASSWORD` | Expense Tracker (docker-compose db init) |