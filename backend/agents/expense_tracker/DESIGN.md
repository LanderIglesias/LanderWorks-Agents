...
### Primary
- **Coral** (`#ff6b35` light / `#ff8a5b` dark): the app's one identity color — "Añadir" action, active tab, primary buttons, sheet-header tint on the alta-manual sheet ("this is new"). Dark mode lightens it for contrast against the near-black ground; it is not the same hex doing double duty.

### Secondary
- **Amber** (`#b8720a` light / `#ffb340` dark): reserved exclusively for `needs_review` — the needs-review dot on a list row, the "Pendiente de revisión" pill, the amber accent dot on a detail sheet's title, and the full amber header tint on the review-queue sheet. Amber never appears for anything the user didn't flag as needing attention.
...
### Buttons
- **Shape:** 14px radius (row scale) for the primary CTA button; true circle for quick-action buttons.
- **Primary:** solid coral fill, white/dark text depending on theme (`--accent-contrast`), 14px vertical padding.
...
- **Do** use amber exclusively for `needs_review` states; never as a second decorative accent.
...
### Don't:
- **Don't** add a second saturated accent color alongside coral and amber — the palette stays two-color plus neutrals.
